...
export class AdminAPIServer {
    ...
    private setupRoutes() {
        ...
        this.app.get('/api/nodes/:nodeId/logs', async (req, res) => {
            ...
            try {
                const { stdout: podNameRaw } = await execAsync(`kubectl get pods -l node-id=${nodeId} -o jsonpath="{.items[0].metadata.name}"`);
                const podName = podNameRaw.trim().replace(/["'\n\r]/g, '');
                if (!podName) return res.status(404).json({ error: 'Pod bulunamadı' });

                const { stdout, stderr } = await execAsync(`kubectl logs ${podName} --tail=500`);
