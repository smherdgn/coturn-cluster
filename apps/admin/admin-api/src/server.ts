...
async function startAdminCluster() {
  try {
    ...
    console.log("");
    console.log(
      `📊 Admin API Server: http://localhost:${
        envConfig.getConfig().ADMIN_API_PORT
      }`
    );
    console.log(
      `🎛️  Dashboard: http://localhost:${envConfig.getConfig().ADMIN_API_PORT}`
    );
    console.log(
      `📡 Pub/Sub: ws://localhost:${envConfig.getConfig().ADMIN_PUBSUB_PORT}`
    );
    console.log(
      `🤖 Agent: http://localhost:${envConfig.getConfig().COTURN_AGENT_PORT}`
    );
    console.log("=============================");

    // Start pub/sub broker (database independent)
    console.log("📡 Starting pub/sub broker...");
    const pubsubBroker = new PubSubBroker(
      envConfig.getConfig().ADMIN_PUBSUB_PORT
    );

    // Start API server
    console.log("📊 Starting API server...");
    const apiServer = new AdminAPIServer(
      envConfig.getConfig().ADMIN_API_PORT,
      pubsubBroker
    );
