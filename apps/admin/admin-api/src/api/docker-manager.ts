...
export class DockerManager {
  async createCoturnNode(
    nodeId: string,
    ip: string,
    ports: { agent: number; turn: number; tls: number }
  ): Promise<void> {
  ...
  async removeCoturnNode(nodeId: string): Promise<void> {
    ...
    try {
      ...
      console.warn(
        `⚠️ Container ${containerName} may not exist or already removed`
      );
    }
  }
  ...
  async getContainerLogs(nodeId: string): Promise<string> {
    ...
    try {
      const { stdout, stderr } = await execAsync(
        `docker logs ${containerName} --tail=500`
      );
      return stdout + "\n" + stderr;
    } catch (error) {
  ...
  async listCoturnNodes(): Promise<string[]> {
    try {
      const { stdout } = await execAsync(
        `docker ps --filter "name=coturn-node-" --format "{{.Names}}"`
      );
      return stdout
        .trim()
        .split("\n")
  ...
  private async waitForContainer(
    ...
    throw new Error(
      `Container ${containerName} failed to start within ${timeoutMs}ms`
    );
  }
